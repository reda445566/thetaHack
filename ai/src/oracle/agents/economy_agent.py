"""Economy expert agent for ORACLE."""
import logging
...
SYSTEM_PROMPT = """You are the ECONOMY AGENT on the ORACLE deliberation council, an expert in economic impact, cost-benefit analysis, jobs, and growth.

The scenario to analyze comes from state["scenario_analysis"] in the user message.

Be critical and independent. Do not defer to other experts. Weigh direct costs against long-run economic effects, flag hidden and opportunity costs, and only support a course of action when the numbers justify it. Use search_latest_data to ground your analysis in current data when relevant.

Produce your final answer through the required structured output schema:
- analysis: str — detailed economic analysis paragraph
...
class EconomyOpinion(BaseModel):
    """Structured output schema for the economy agent."""
    analysis: str
    key_risks: list[str]
...
def _fallback_opinion(error: str) -> dict:
    ...
    return {
        "analysis": f"Economic analysis unavailable ({error}).",
        "key_risks": ["Unable to assess economic risks."],
...
def economy_agent(state: OracleState) -> OracleState:
    """Analyze the scenario from an economic impact perspective."""
    try:
        ...
        for _ in range(MAX_TOOL_ROUNDS):
            ...
            response = tool_model.invoke(messages)
        opinion_model = llm.with_structured_output(EconomyOpinion)
        opinion = opinion_model.invoke(messages).model_dump()
    except Exception as e:
        logger.exception("Economy agent failed")
        opinion = _fallback_opinion(str(e))
    return {
        **state,
        "expert_opinions": {**state.get("expert_opinions", {}), "economy": opinion},
    }
